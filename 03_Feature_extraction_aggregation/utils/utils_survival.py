...
def collate_MIL_survival(batch):
    ...
    label = torch.FloatTensor([item[2] for item in batch])  # NOTE: 修订，每个病人一个切片
...
def get_split_loader_surv(split_dataset, training = False, testing = False, weighted = False, mode='coattn', batch_size=1):
    """
...
def ce_loss(hazards, S, Y, c, alpha=0.4, eps=1e-7):
    ...
    reg = -(1 - c) * (torch.log(torch.gather(S_padded, 1, index=Y)+eps) + torch.log(torch.gather(hazards, 1, index=Y).clamp(min=eps)))
    ce_l = - c * torch.log(torch.gather(S, 1, index=Y).clamp(min=eps)) - (1 - c) * torch.log(1 - torch.gather(S, 1, index=Y).clamp(min=eps))
