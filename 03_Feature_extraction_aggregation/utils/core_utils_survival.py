...
def train_survival(datasets: tuple, cur: int, args: Namespace):
    ...
    print('\nInit Loaders...', end=' ')
    train_loader = get_split_loader_surv(train_split, training=True, testing = args.testing, weighted=args.weighted_sample,
                                         mode=args.mode, batch_size=args.batch_size)
    val_loader = get_split_loader_surv(val_split,  testing = args.testing, mode=args.mode, batch_size=args.batch_size)
    print('Done!')
    ...
    model.load_state_dict(torch.load(os.path.join(args.results_dir, "s_{}_checkpoint.pt".format(cur)), weights_only=True)) # NOTE: 官方警告
...
def train_loop_survival(epoch, model, loader, optimizer, n_classes, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., gc=16):
    ...
    for batch_idx, (data_WSI, data_omic, y_disc, event_time, censor) in enumerate(loader):
        ...
        # pdb.set_trace()
        if y_disc.shape[0] == 1 and (batch_idx + 1) % 20 == 0:
