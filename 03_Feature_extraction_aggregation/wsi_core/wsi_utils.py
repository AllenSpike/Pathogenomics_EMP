...
def DrawMapFromCoords(canvas, wsi_object, coords, patch_size, vis_level, indices=None, verbose=1, draw_grid=True):
    ...
    downsamples = wsi_object.level_downsamples[vis_level][0] # note: 修订
    if indices is None:
        indices = np.arange(len(coords))
    total = len(indices)
    if verbose > 0:
    ...
    for idx in range(total):
        ...
        patch = np.array(wsi_object.readregion(tuple(coord), vis_level, patch_size))  # note: 修订
...
def StitchCoords(hdf5_file_path, wsi_object, downscale=16, draw_grid=False, bg_color=(0,0,0), alpha=-1, best_level = None): # note:修订best_level参数
    ...
    # w, h = wsi_object.level_dim[vis_level] # note: 修订
    ...
    # patch_size = tuple((np.array((patch_size, patch_size)) * wsi_object.level_downsamples[patch_level][0]).astype(np.int32)) # note: 修订
    ...
    if alpha < 0 or alpha == -1:
        heatmap = Image.new(size=(w,h), mode="RGB", color=bg_color)
        # heatmap = Image.new(size=(h, w), mode="RGB", color=bg_color)  # note: 修订
    else:
        heatmap = Image.new(size=(w,h), mode="RGBA", color=bg_color + (int(255 * alpha),))
        # heatmap = Image.new(size=(h, w), mode="RGBA", color=bg_color + (int(255 * alpha),))  # note: 修订
