...
import h5py
import math
import geopandas as gpd # note: 修订
...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
        ...
        if bool(re.match(r".*\.ome$", self.name)):
            self.ome = tifffile.TiffFile(path)
            self.level_dim = tuple([(x.shape[0], x.shape[1]) for x in self.ome.series[0].levels])
            self.level_downsamples = tuple([self.level_dim[0][0]/x[0] for x in self.level_dim])
            self.level_downsamples = self._assertLevelDownsamples2() # note: 修订
        else :
    ...
    def initGeojson(self,annot_path): # note: 修订
        def _create_contours_from_dict(annot):
            all_cnts = []
            for idx, annot_group in enumerate(annot):
                contour_group = annot_group.exterior.coords
                if annot_group.geom_type == 'Polygon':
                    for idx, contour in enumerate(contour_group):
                        contour = np.array(contour).astype(np.int32).reshape(-1, 1, 2)
                        all_cnts.append(contour)
        ...
        annot = gpd.read_file(annot_path)
        annot = annot['geometry']
        self.contours_tumor = _create_contours_from_dict(annot)
        self.contours_tumor = sorted(self.contours_tumor, key=cv2.contourArea, reverse=True)
    ...
    def readregion(self, location, level, size): # note: 修订
        if bool(re.match(r".*\.ome$", self.name)):
            downsamples = self.level_downsamples[level][0]
            region = PIL.Image.fromarray(self.ome.series[0].levels[level].asarray()[int(np.ceil(location[1] / downsamples)):int(np.ceil(location[1] / downsamples) + size[0]),
                                                                                    int(np.ceil(location[0] / downsamples)):int(np.ceil(location[0] / downsamples) + size[1])])
        else:
            region = self.wsi.read_region(location, level, size).convert("RGB")
        return region
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
        ...
        img = np.array(self.readregion((0, 0), seg_level, self.level_dim[seg_level])) # note：修订
    ...
    def visWSI(self, vis_level=0, color = (0,255,0), hole_color = (0,0,255), annot_color=(255,0,0), 
        ...
        img = np.array(self.readregion(top_left, vis_level, region_size).convert("RGB")) # note: 修订
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256, custom_downsample=1,
        ...
        for y in range(start_y, stop_y, step_size_y):
            for x in range(start_x, stop_x, step_size_x):
                ...
                patch_PIL = self.readregion((x, y), patch_level, (patch_size, patch_size)).convert('RGB')  # note: 修订
                if custom_downsample > 1:
    ...
    def _assertLevelDownsamples2(self): # note:修订
        level_downsamples = []
        dim_0 = self.level_dim[0]

        for downsample, dim in zip(self.level_downsamples, self.level_dim):
            estimated_downsample = (dim_0[0] / float(dim[0]), dim_0[1] / float(dim[1]))
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        ...
        print("Creating patches for: ", self.name, "...",)
        elapsed = time.time()
        n_contours = len(self.contours_tissue) # 当前wsi中有多少个轮廓
        print("Total number of contours to process: ", n_contours)
        fp_chunk_size = math.ceil(n_contours * 0.05)
        init = True
        for idx, cont in enumerate(self.contours_tissue): # 每个轮廓依次处理
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        print("Bounding Box:", start_x, start_y, w, h) # 边界框的坐g标
        print("Contour Area:", cv2.contourArea(cont)) # 轮廓面积
        ...
        if isinstance(contour_fn, str): # 实例化函数：检查patch是否应该被视为前景或者背景
            if contour_fn == 'four_pt':
                cont_check_fn = isInContourV3_Easy(contour=cont, patch_size=ref_patch_size[0], center_shift=0.5)
            elif contour_fn == 'four_pt_hard':
                # cont_check_fn = isInContourV3_Hard(contour=cont, patch_size=ref_patch_size[0], center_shift=0.5)
                cont_check_fn = isInContourV3_Hard(contour=cont, patch_size=ref_patch_size[0], center_shift=0.8) # 修订
            elif contour_fn == 'center':
                cont_check_fn = isInContourV2(contour=cont, patch_size=ref_patch_size[0])
            elif contour_fn == 'basic':
                cont_check_fn = isInContourV1(contour=cont)
        ...
        iterable = [(coord, contour_holes, ref_patch_size[0], cont_check_fn) for coord in coord_candidates] # 构建迭代对象（patch坐标、洞轮廓、patch size、判断patch为前/背景的实例化函数）
        results = pool.starmap(WholeSlideImage.process_coord_candidate, iterable) # 并行化，同时检查patch是是否为前/背景，以及是否在洞中
        pool.close()
