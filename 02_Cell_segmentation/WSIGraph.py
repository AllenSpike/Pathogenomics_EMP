...
def getSingleGraphFeatures(args):
    subgraph, cmd = args
    result = defaultdict(list)
    n = subgraph.vcount() # 亚子图中没有边连接的节点不被计数
    if cmd == 'name':
        result['name'] += [int(i) for i in subgraph.vs['name']] # 将亚子图的节点存入name
    elif cmd == 'Nsubgraph':
        result['Nsubgraph'] += [n] * n # 亚子图中所有节点的Nsubgraph属性均一致
    elif cmd == 'Degrees':
...
def getGraphCenterFeatures(graph: ig.Graph):
    ...
    norm_cmds = ['name', 'Nsubgraph', 'Degrees',
                 ...
                 'Coreness', 'ClusteringCoefficient'] # 单线程计算的特征
    multi_cmds = ['Eccentricity', 'HarmonicCentrality', 'Closeness', 'Betweenness'] # 多线程计算的特征

    i = 1 # crz修改

    for subgraph in tqdm(graph.decompose()): # 按照连通性将输入的子图进一步分解为多个亚子图
        ...
        result['sub-subgraph'] += [i] * subgraph.vcount() # 获取亚子图编号 crz修改
        i = i + 1 # crz修改

    return result # 长度为子图节点数目
...
def constructGraphFromDict(
    ...
    globalGraph.add_vertices(vertex_len, attributes={
        ...
        'Contour': contours, 'CellType': types})  # 向图中添加细胞

    # 1、提取形态特征
    print('Getting morph features')
    t1 = time.time()
    morphFeats = getMorphFeatures(names, contours, bboxes, 'MorphFeatures', process_n=8)
    # morphFeats = getMorphFeatures(names, contours, bboxes, 'MorphFeatures', process_n=4)
    ...
    # GLCMFeats = getGLCMFeatures(wsiPath, names, contours, bboxes, pad=2, level=level, process_n=4)
    ...
    edge_info = pd.DataFrame({'source': [], 'target': [], 'length' : [], 'featype': []}) # crz修改
    ...
    for featype, featype_index_list in zip(featype_dict.keys(),
        ...
        for src, i_src in enumerate(featype_index_list): # source细胞类型和对应的索引
            for tar, i_tar in enumerate(featype_index_list): # target细胞类型和对应的索引
                if src != tar:
                    ...
                    tree = cKDTree(centroid_tar) # 根据target像素坐标构建CKD树

                    if i_src == i_tar: # 如果source和target的坐标均相同
                        disknn, vertex_index = tree.query(centroid_src, k=knn_n + 1,
                                                          distance_upper_bound=distanceThreshold, p=2) # 寻找距离每个source最近的6个target，使用欧几里得距离，距离上限为40个像素，超出设置为inf，相应的索引被设置为索引最大值
                    ...
                    v_src = np.tile(np.array(i_src, dtype='str').reshape((n_src, -1)), (1, knn_n))[knn_mask] # 创建source的一维数组
                    v_tar = np.array(i_tar, dtype='str')[vertex_index[knn_mask]] # 将target压缩一维数组
                    v_len = disknn[knn_mask] # 边距离

                    # pairs = np.concatenate([pairs, np.stack((v_src, v_tar), axis=1)], axis=0) # 边关系表
                    pairs = np.concatenate([pairs, np.stack((v_src, v_tar, v_len), axis=1)], axis=0) # 带有距离的边关系表、crz修改
                    disKnnList = np.concatenate([disKnnList, disknn], axis=0) # 距离表 -》其实也可以输出
                    subgraph_names += i_src

        subgraph = globalGraph.induced_subgraph(featype_index) # 提取包含source和target的子图
        ...
        subgraph_edge.insert(subgraph_edge.shape[1], 'length', [np.float16(x) for x in pairs[:,2]])
        edge_info = pd.concat([edge_info, subgraph_edge]) # 边信息的汇总表
