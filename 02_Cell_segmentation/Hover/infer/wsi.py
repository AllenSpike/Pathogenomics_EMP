...
import slideflow as sf ## note: 导入slideflow进行qc并对chunk进行归一化
...
# 2021-11-18
def _post_proc_para_wrapper_backup(wsi_pred_map, tile_info, func, func_kwargs):
    """Wrapper for parallel post processing."""
    idx, tile_tl, tile_br = tile_info
    tile_pred_map = wsi_pred_map[tile_tl[0] : tile_br[0], tile_tl[1] : tile_br[1]]
    return func(tile_pred_map, **func_kwargs), tile_info
...
# 2021-11-18
def _assemble_and_flush_backup(wsi_pred_map, chunk_info, patch_output_list):
    """Assemble the results. Write to newly created holder for this wsi"""
    chunk_pred_map = wsi_pred_map[
        chunk_info[1][0][0] : chunk_info[1][1][0],
        chunk_info[1][0][1] : chunk_info[1][1][1],
    ]
    if patch_output_list is None:
...
class InferManager(base.InferManager):
    ...
    def __get_raw_prediction(self, chunk_info_list, patch_info_list
        ...
        for idx in range(0, chunk_info_list.shape[0]):
            ...
            if chunk_patch_info_list.shape[0] == 0:
                ...
                # ! 2021-11-18
                # proc_pool.apply_async(
                #     _assemble_and_flush, args=(self.wsi_pred_map, chunk_info, None)
                # )
                continue
            ...
            # chunk_data = model.transform(chunk_data)  # note: 在将图像写进缓存之前使用slideflow进行归一化

            np.save("%s/cache_chunk.npy" % self.cache_path, chunk_data) # 将RGB图像写进缓存
            ...
            # proc_pool.apply_async(
            #     _assemble_and_flush,
            #     args=(self.wsi_pred_map, chunk_info, patch_output_list),
            # )
        proc_pool.close()
        proc_pool.join()
        return
    ...
    def __dispatch_post_processing(self, tile_info_list, callback):
        ...
        for idx in list(range(tile_info_list.shape[0])):
            ...
            if proc_pool is not None:
                ...
                # ! 2021-11-18
                # results = _post_proc_para_wrapper(
                #     self.wsi_pred_map, tile_info, self.post_proc_func, func_kwargs
                # )
                callback(results)
        if proc_pool is not None:
            silent_crash = False
    ...
    def process_single_file(self, wsi_path, msk_path, output_dir
        ...
        ambiguous_size = self.ambiguous_size # 128
        tile_shape = (np.array(self.tile_shape)).astype(np.int64) # 2048
        chunk_input_shape = np.array(self.chunk_shape) # 10000
        patch_input_shape = np.array(self.patch_input_shape) # 256
        patch_output_shape = np.array(self.patch_output_shape) # 164
        ...
        if msk_path is not None and os.path.isfile(msk_path): # note: 导入掩码
    ...
    def process_wsi_list(self, run_args):
        ...
        wsi_path_list = glob.glob(self.input_dir + "/*") # note: 修订
        wsi_path_list.sort()  # ensure ordering

        # # note：导入归一化模型，并在tcga数据集上进行初始化
        # dataset_ref = sf.Dataset(
        #     tfrecords='/home/ljc/0_project/0_ESCC/0_scMTOP/data_tcga/tfrecords',
        #     slides='/home/ljc/0_project/0_ESCC/0_scMTOP/data_tcga/slides',
        #     tiles='/home/ljc/0_project/0_ESCC/0_scMTOP/data_tcga/tiles',
        #     annotations='/home/ljc/0_project/0_ESCC/0_scMTOP/data_tcga/annotations.csv',
        #     tile_px=299,
        #     tile_um='10x'
        # )
